...
namespace CalculatorProject
{
    public partial class MainPage : ContentPage
    {
        ...
        void onOperatorSelection(object sender, EventArgs e)
        {
            ...
            if (currentState == 2) // Eğer ikinci sayı girildiyse işlemi tamamla
            {
                firstNum = Calculate.DoCalculation(firstNum, secondNum, operatorMath);
                this.result.Text = firstNum.ToString();
                secondNum = 0; // İkinci sayıyı sıfırla
            }

            operatorMath = btnPressed; // Yeni operatörü kaydet
            currentState = -2; // İkinci sayıyı bekle
        }
        ...
        void onCalculate(object sender, EventArgs e)
        {
            if (currentState == 2)
            {
                ...
                this.result.Text = firstNum.ToString() + " " + operatorMath + " " +  secondNum.ToString() + " = " + result.ToString() + " ";
                firstNum = result;
                currentState = -1;
            }
        }
